...
class PhiTraining(Task):
    ...
    def optimize_delta(self, h_prev_cache, target_str):
        ...
        # Assuming h_prev_cache corresponds to context processing
        # cache_pos needs to be correct relative to context length
        # We need the context length. 
        # In this task flow, we'll get h_prev_cache from a context run.
    ...
    def run(self):
        ...
        # We need base cache structure for forward pass loop
        probe_ids = self.tokenizer(self.probe_text, return_tensors="pt").input_ids.to(self.device)
        context_ids = probe_ids[:, :-1]
        ...
        for epoch in range(101):
            ...
            if epoch % 20 == 0:
                print(f"  Epoch {epoch}: Loss {total_loss/len(self.dataset):.4f}")
                
        torch.save(self.phi.state_dict(), self.phi_path)
        print(f"[{self.name}] Phi model saved.")

    def report(self):
        pass # Could generate eval table
