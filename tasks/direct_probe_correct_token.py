...
class DirectProbeCorrectToken(Task):
    def __init__(self):
        super().__init__("DirectProbeCorrectToken")
        self.layer_idx = 7
        self.results = []
    ...
    def optimize_delta(self, h_prev_cache, target_str, context_len, last_token_id):
        ...
        # USE THE CORRECT INPUT TOKEN
        cache_pos = torch.tensor([context_len], device=self.device)
        
        print(f"    Optimizing delta for '{target_str}' using correct input token...")
    ...
    def run_case(self, prompt, target):
        ...
        print(f"\n--- Testing (Corrected): '{prompt}' -> {target} ---")
        ...
        # Capture the actual last token to use as input for next step optimization
        last_token_id = probe_ids[:, -1:]
        
        delta, rank = self.optimize_delta(h_prev_cache, target, context_len, last_token_id)
    ...
    def run(self):
        print(f"[{self.name}] Running Direct Probe (Corrected Token)...")
        for prompt, target in self.cases:
            self.run_case(prompt, target)
            
        with open(os.path.join(self.output_dir, "direct_probe_corrected.json"), "w") as f:
