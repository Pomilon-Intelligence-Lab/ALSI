...
class DistributedInjectionTest(Task):
    def __init__(self):
        super().__init__("DistributedInjectionTest")
        self.start_layer = 16
        self.end_layer = 24
    ...
    def run(self):
        print(f"[{self.name}] Running Distributed Injection (Layers {self.start_layer}-{self.end_layer-1})...")
        ...
        print(f"  Optimizing field across {len(deltas)} layers...")
        ...
        for step in range(50):
            ...
            loss = F.cross_entropy(logits.view(1, -1), torch.tensor([target_id], device=self.device)) + 1e-4 * total_norm
            loss.backward()
            optimizer.step()
            ...
            with torch.no_grad():
                ...
                if rank == 1:
                    print(f"    Hit Rank 1 at step {step} (Avg Norm: {total_norm.item() / len(deltas):.2f})")
                    break
        
        # 4. Verification
        print("\n--- Distributed Injection Verification ---")
        # Apply all deltas
        for i in range(self.start_layer, self.end_layer):
            delta_idx = i - self.start_layer
