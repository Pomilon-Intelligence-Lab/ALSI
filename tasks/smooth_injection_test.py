...
class SmoothInjectionTest(Task):
    def __init__(self):
        super().__init__("SmoothInjectionTest")
        self.start_layer = 12
        self.end_layer = 24
    ...
    def run(self):
        print(f"[{self.name}] Running Smooth Distributed Injection (Layers {self.start_layer}-{self.end_layer-1})...")
        ...
        print(f"  Optimizing smooth field across {len(deltas)} layers...")
        ...
        for step in range(50):
            ...
            for i in range(self.start_layer, self.end_layer):
                ...
                # Smoothness: Penalize diff between adjacent layers
                if delta_idx > 0:
            ...
            # Combined Loss
            # We want: 
            # 1. Force token (l_target)
            # 2. Stay near natural (l_kl)
            # 3. Be smooth across depth (smooth_loss)
            loss = l_target + 0.1 * l_kl + 1.0 * smooth_loss + 1e-4 * total_norm
            ...
            with torch.no_grad():
                ...
                if rank == 1:
                    print(f"    Hit Rank 1 at step {step} (KL: {l_kl.item():.4f}, Smooth: {smooth_loss.item():.4f})")
                    break
        
        # 4. Verification
        print("\n--- Smooth Injection Verification ---")
        # Apply all deltas
        for i in range(self.start_layer, self.end_layer):
            delta_idx = i - self.start_layer
